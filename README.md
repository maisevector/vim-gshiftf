...
`gf` reads the `<cfile>` expression. The settings of `isfname` define, which ASCII characters are valid file path characters. If you wish to modify the valid characters (e.g. to include whitespace), use `:set isfname+=32`. For further reading, we recommend `:help <cfile>` and `:help isfname`
